...
public record OverlayRule(
    ...
    BigDecimal minAdjustment,
    BigDecimal maxAdjustment,
    int priority,
    ...
    public OverlayRule {
        ...
        exclusivityGroup = blankToNull(exclusivityGroup);
    }
    ...
    boolean scopeMatches(OverlayInputs inputs) {
        return matches(investorCode, inputs.investorCode())
            && matches(channelCode, inputs.channelCode())
            && matches(productFamily, inputs.productFamily())
            && matches(loanPurpose, inputs.loanPurpose())
            && matches(occupancy, inputs.occupancy())
            && matches(propertyType, inputs.propertyType())
            && matches(stateCode, inputs.stateCode())
            && matches(countyCode, inputs.countyCode());
    }
