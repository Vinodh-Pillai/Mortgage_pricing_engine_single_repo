...
const capabilityDefinitions: CapabilityDefinition[] = [
  ...
  { id: 'quick-quote', label: 'New quote', group: 'Pipeline', icon: '⚡', moduleId: 'quickquote', route: '/quote/start' },
...
function itemForCapability(definition: CapabilityDefinition, moduleById: Map<string, WorkbenchScreenModule>, activeRunId?: string | null): NavigationItem {
...
function itemForModule(module: WorkbenchScreenModule, activeRunId?: string | null): NavigationItem {
...
export function buildNavigationTree(modules: WorkbenchScreenModule[], activeRunId?: string | null, persona?: string | Persona | null): NavigationItem[] {
  ...
  const capabilityItems = capabilityDefinitions
    .map((definition) => itemForCapability(definition, moduleById, activeRunId))
  ...
  const registryFallbackItems = modules
    ...
    .map((module) => itemForModule(module, activeRunId))
    .filter((item) => isVisibleNavigationItem(item, personaObject));
