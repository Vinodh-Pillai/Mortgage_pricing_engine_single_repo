...
export const workbenchModules: WorkbenchScreenModule[] = [
  ...
  },
  {
    ...
    stateCoverage: ['load-state', 'empty', 'ready', 'blocked'],
    ...
    evidenceTarget: '.local-harness/evidence/PII-53-S01/product-admin.json',
  ...
  },
  {
    ...
    dataBoundary: 'catalog-service admin product and pricing-profile reference APIs',
    stateCoverage: ['loading', 'empty', 'blocked', 'needs-attention', 'ready'],
    ...
    match: (pathname) => pathname.startsWith('/admin/products/catalog') || pathname.startsWith('/admin/products/management') || pathname.startsWith('/admin/products/new'),
  },
  {
    id: 'investor-management',
    label: 'Investor Management',
    routePattern: '/admin/investors',
    breadcrumb: 'Investor Management',
    screenPackage: 'screens/investorManagement',
    dataBoundary: 'catalog-service investor, product/channel mapping, and pricing-profile reference APIs',
    stateCoverage: ['ready', 'empty', 'bulk-selection', 'detail-slide-over', 'create-slide-over'],
    personaVisibility: ['admin', 'pricing-analyst', 'operations-lead'],
    dependencyStatus: 'Catalog-service and tenant-context-service own persisted investor profiles and mappings.',
    adapterStatus: 'Local UI captures setup intent without inventing rates, fees, thresholds, or eligibility rules.',
    evidenceTarget: '.local-harness/evidence/direct-investor-management/investor-management.json',
    match: (pathname) => pathname === '/admin/investors' || pathname.startsWith('/admin/investors/'),
  },
  {
    id: 'pricing-profiles',
    label: 'Pricing Profiles',
    routePattern: '/pricing/profiles',
    breadcrumb: 'Pricing Profiles',
    screenPackage: 'screens/productManagement',
    dataBoundary: 'catalog-service pricing profile references and product pricing setup APIs',
    stateCoverage: ['loading', 'empty', 'blocked', 'needs-attention', 'ready'],
    personaVisibility: ['admin', 'pricing-analyst'],
    dependencyStatus: 'Backend profile APIs own persisted profile rules and values; the screen shows setup references only.',
    adapterStatus: 'Pricing profile navigation reuses product setup surfaces until a dedicated backend contract is available.',
    evidenceTarget: '.local-harness/evidence/direct-routing-test-fixes/pricing-profiles.json',
    match: (pathname) => pathname === '/pricing/profiles' || pathname.startsWith('/pricing/profiles/'),
  },
  {
  ...
  },
  {
    id: 'rules-engine',
    label: 'Rules Engine',
    routePattern: '/rules-engine',
    breadcrumb: 'Rules Engine',
    screenPackage: 'screens/adminGovernance',
    dataBoundary: 'governance and dynamic-rule evidence APIs',
    stateCoverage: ['load-state', 'descriptors', 'pending-review', 'dynamic-rule-evidence', 'blocked', 'ready'],
    personaVisibility: ['admin', 'pricing-analyst', 'governance-reviewer'],
    dependencyStatus: 'Rules engine persistence and approvals remain backend-owned governance references.',
    adapterStatus: 'Rules navigation opens governance evidence without hardcoding mortgage pricing policy.',
    evidenceTarget: '.local-harness/evidence/direct-routing-test-fixes/rules-engine.json',
    match: (pathname) => pathname === '/rules-engine' || pathname.startsWith('/rules-engine/'),
  },
  {
    id: 'user-management',
    ...
    screenPackage: 'screens/tenantAdmin',
    dataBoundary: 'tenant-context-service user access and persona administration APIs',
    stateCoverage: ['loading', 'empty', 'blocked', 'needs-attention', 'ready'],
    personaVisibility: ['admin'],
    dependencyStatus: 'Tenant-context-service owns persisted users, roles, and persona access.',
    adapterStatus: 'User management navigation reuses tenant administration until dedicated user APIs are available.',
    evidenceTarget: '.local-harness/evidence/direct-routing-test-fixes/user-management.json',
