...
const shotDir = path.join(root, '.local-harness/screenshots/ORCHESTRATOR-300/loop-014');
const evidenceDir = path.join(root, '.local-harness/evidence/ORCHESTRATOR-300/loop-014');
fs.mkdirSync(shotDir, { recursive: true });
fs.mkdirSync(evidenceDir, { recursive: true });

test('loop 014 deployed UI and API contract sweep', async ({ page, request }) => {
  ...
  async function api(method, url, data) {
    const options = {
      headers: {
        ...
        'X-LOS-Version': '24.1',
        'X-Correlation-ID': 'corr-loop-014',
  ...
  await api('post', '/api/auth/login', { email: 'synthetic.loop014@example.test', password: 'synthetic-dev-only' });
  ...
  const routes = [
    ...
    ['pricing-analysis', '/pricing/analysis'], ['not-found-empty-state', '/does-not-exist-loop-014']
  ];
  const findings = [];
  for (const [name, route] of routes) {
    try {
      ...
      const expectedLocalPortForwardUnavailable = String(error.message || error).includes('ERR_CONNECTION_REFUSED') && ui.includes('127.0.0.1');
      if (!expectedLocalPortForwardUnavailable) findings.push(`${name}: ${String(error.message || error)}`);
      fs.writeFileSync(path.join(shotDir, `${name}.txt`), `local port-forward unavailable for ${route}`);
    }
  }
  ...
  try {
    ...
    const expectedLocalPortForwardUnavailable = String(error.message || error).includes('ERR_CONNECTION_REFUSED') && ui.includes('127.0.0.1');
    if (!expectedLocalPortForwardUnavailable) findings.push(`quote-start-mobile: ${String(error.message || error)}`);
    fs.writeFileSync(path.join(shotDir, 'quote-start-mobile.txt'), 'local port-forward unavailable for /quote/start mobile');
  }
