...
const shotDir = path.join(root, '.local-harness/screenshots/ORCHESTRATOR-300/loop-015');
const evidenceDir = path.join(root, '.local-harness/evidence/ORCHESTRATOR-300/loop-015');
fs.mkdirSync(shotDir, { recursive: true });
fs.mkdirSync(evidenceDir, { recursive: true });

test('loop 015 deployed UI and API contract sweep', async ({ page, request }) => {
  ...
  async function api(method, url, data) {
    const options = {
      headers: {
        ...
        'X-LOS-Version': '24.1',
        'X-Correlation-ID': 'corr-loop-015',
  ...
  await api('post', '/api/auth/login', { email: 'synthetic.loop015@example.test', password: 'synthetic-dev-only' });
  ...
  const routes = [
    ...
    ['pricing-analysis', '/pricing/analysis'], ['not-found-empty-state', '/does-not-exist-loop-015']
  ];
  const findings = [];
