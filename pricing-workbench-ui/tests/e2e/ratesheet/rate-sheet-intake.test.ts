...
test.describe('PII-25 rate sheet intake page', () => {
  test('covers drag-drop upload, validation, grid, publish workflow, visual and a11y checks', async ({ page }) => {
    ...
    await expectMajorFunctionalityPage(page, 'Rate Sheet Intake', ['Upload/Import', 'Validation Results', 'Rate Grid', 'Publish Workflow']);
    await page.getByLabel(/Rate sheet file/i).setInputFiles({ name: 'pii25-rate-sheet.csv', mimeType: 'text/csv', buffer: Buffer.from('term,rate\n360,service-owned\n') });
    await expect(page.getByText(/uploaded 100%/i)).toBeVisible();
